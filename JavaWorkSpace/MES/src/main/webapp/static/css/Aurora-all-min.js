...
/**
 * 灏嗗璞″眳涓�
 * @param {Object/String} el Aurora缁勪欢瀵硅薄鎴栬€呮槸DOM瀵硅薄鎴栬€呮槸瀵硅薄鐨処D瀛楃涓�
 */
...
/**
 * 鑾峰彇Aurora鎺т欢鐨勫璞★紝鍙互浣跨敤绠€鍐欐柟寮忕殑$()鏂规硶
 * @param {String} id Aurora鎺т欢鐨刬d
 */
$ = $A.getCmp = function(id){
    var cmp = $A.CmpManager.get(id)
    if(cmp == null) {
//        alert('鏈壘鍒扮粍浠�:' + id)
        window.console && console.error('鏈壘鍒扮粍浠�:' + id);
    }
    return cmp;
}
 ...
 * @param {Number} days 鏈夋晥鏈�(鍗曚綅鏄ぉ),榛樿鏄痵essions
 */
...
//$A.recordSize();
/**
 * post鐨勬柟寮忔彁浜ゆ暟鎹紝鍚寋@link Aurora.DataSet#post}
 * @param {String} action 鎻愪氦鐨剈rl鍦板潃
 * @param {Object} data 鏁版嵁闆嗗悎
 * @param {String} target 鎻愪氦鐩爣
 */
$A.post = function(action,data,target){
    ...
    for(var key in data){
        var v = data[key]
        if(v) {
            if(v instanceof Date) v = v.format('isoDate');//TODO:鏃跺垎绉掑浣曞鐞�?
 ...
 * <li><code>url</code>
 * <div class="sub-desc">鎻愪氦鐨剈rl鍦板潃</div></li>
 * <li><code>para</code>
 * <div class="sub-desc">鎻愪氦鐨勫弬鏁�</div></li>
 * <li><code>scope</code>
 * <div class="sub-desc">浣滅敤鍩�</div></li>
 * <li><code>sync</code>
 * <div class="sub-desc">鏄惁鍚屾,榛樿false</div></li>
 * <li><code>success</code>
 * <div class="sub-desc">鎴愬姛鐨勫洖璋冨嚱鏁�</div></li>
 * <li><code>error</code>
 * <div class="sub-desc">閿欒鐨勫洖璋冨嚱鏁�</div></li>
 * <li><code>failure</code>
 * <div class="sub-desc">ajax璋冪敤澶辫触鐨勫洖璋冨嚱鏁�</div></li>
 * <li><code>lockMessage</code>
 * <div class="sub-desc">閿佸睆娑堟伅</div></li>
 * </ul></div></p>
 * @param {Object} opt 鍙傛暟瀵硅薄
 */
...
$A.Masker = function(){
    var m = {
        ...
        mask : function(el,msg){
            ...
            var el = Ext.get(el);
            var w = el.getWidth();
            var h = el.getHeight();//leftp:0px;top:0px; 鏄惁寮曡捣resize?
        ...
        try {
            for(j=0,k=jsscript.length;j<k;j++){
                ...
                try {
                    ...
                    window.console && console.error("鎵ц浠ｇ爜: " + jst +'\n'+e.stack);
                }
            }
        }catch(e){}
...
//        Ext.fly(dom).setStyle('display', 'block');
    ...
    var onErrorLoad = function(evt){
        loaded++;
        alert('鏃犳硶鍔犺浇鑴氭湰:' + evt.target.src);
        if(loaded==jslink.length) {
            finishLoad();
        }else {
            continueLoad();
...
/**
 * 灏嗘棩鏈熻浆鎹㈡垚榛樿鏍煎紡鐨勫瓧绗︿覆锛岄粯璁ゆ牸寮忔槸鏍规嵁Aurora.defaultDateFormat鏉ュ畾涔夌殑.濡傛灉娌℃湁鐗规畩鎸囧畾,榛樿鏍煎紡涓簓yyy-mm-dd
 * @param {Date} date 杞崲鐨勬棩鏈�
 * @return {String}
 */
...
/**
 * 灏嗘棩鏈熻浆鎹㈡垚yyyy-mm-dd HH:MM:ss鏍煎紡鐨勫瓧绗︿覆
 * @param {Date} date 闇€瑕佽浆鎹㈢殑鏃ユ湡
...
/**
 * 灏嗘暟鍊兼牴鎹簿搴﹁浆鎹㈡垚甯︽湁鍗冨垎浣嶇殑瀛楃涓�
 *
 * @param {Number} value 鏁板€�
 * @param {Number} decimalprecision 灏忔暟鐐逛綅鏁�
 * @return {String}
 */
...
/**
 * 灏嗘暟鍊艰浆鎹㈡垚甯︽湁鍗冨垎浣嶇殑瀛楃涓诧紝骞朵繚鐣欎袱浣嶅皬鏁�
 *
 * @param {Number} value 鏁板€�
 * @return {String}
 */
 ...
 * 灏嗗瓧绗︿覆鐨勫崈鍒嗕綅鍘婚櫎
 * @param {Number} value 鏁板€�
 * @param {String} rv 甯︽湁鍗冨垎浣嶇殑鏁板€煎瓧绗︿覆
 * @return {Number} 鏁板€�
 */
...
$A.convertMoney = function(mnum){
    mnum = Math.abs(mnum);
    var unitArray = [["鍏�", "涓�", "浜�"], ["浠�", "", "鎷�", "浣�"],["闆�", "澹�", "璐�", "鍙�", "鑲�", "浼�", "闄�", "鏌�", "鎹�", "鐜�"]];
    ...
    strout = strout.replace(new RegExp(/闆�+/g), "闆�");
    strout = strout.replace("闆朵竾", "涓�");
    strout = strout.replace("闆朵嚎", "浜�");
    strout = strout.replace("闆跺厓", "鍏�");
    strout = strout.replace("浜夸竾", "浜�");
    var strdec = ""
    if (decimalpart[0] == 0 && decimalpart[1] == 0) {
        ...
        if (decimalpart[0] == 0) {
            ...
            strdec = unitArray[2][decimalpart[0]] + '瑙�';
        }
        if (decimalpart[1] != 0) {
            strdec += unitArray[2][decimalpart[1]] + '鍒�';
        }
    }
    strout += strdec;
    if (mnum < 0)
 ...
 * <p>DataSet鏄竴涓暟鎹簮锛屼篃鏄竴涓暟鎹泦鍚堬紝瀹冨皝瑁呬簡鎵€鏈夋暟鎹殑鎿嶄綔锛屾牎楠岋紝鎻愪氦绛夋搷浣�.
...
$A.DataSet = Ext.extend(Ext.util.Observable,{
    constructor: function(config) {//datas,fields, type
        ...
        if(config.datas && config.datas.length != 0) {
            ...
            this.loadData(datas);
//            $A.onReady(function(){
//        sf.locate(sf.currentIndex,true); //涓嶇‘瀹氭湁娌℃湁褰卞搷
//            });
        }
...
//        this[ou]('beforecreate', this.beforeCreate, this);//TODO:鏈夊緟娴嬭瘯
    ...
    /**
     * 灏嗙粍浠剁粦瀹氬埌鏌愪釜DataSet鐨勬煇涓狥ield涓�.
     * @param {Aurora.DataSet} dataSet 缁戝畾鐨凞ataSet.
     * @param {String} name Field鐨刵ame.
     *
     */
...
//        c.bindname = this.bindname;
    ...
    initEvents : function(){
        this.addEvents(
             ...
             * @event ajaxfailed
             * ajax璋冪敤澶辫触.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
            ...
            /**
             * @event beforecreate
             * 鏁版嵁鍒涘缓鍓嶄簨浠�.杩斿洖true鍒欐柊澧炰竴鏉¤褰�,false鍒欎笉鏂板鐩存帴杩斿洖
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Object} object 鏂板鐨勬暟鎹璞�.
             */
            'beforecreate',
            /**
             * @event metachange
             * meta閰嶇疆鏀瑰彉浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 褰撳墠鐨剅ecord.
             * @param {Aurora.Record.Meta} meta meta閰嶇疆瀵硅薄.
             ...
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 褰撳墠鐨剅ecord.
             * @param {Aurora.Record.Field} field Field閰嶇疆瀵硅薄.
             * @param {String} type 绫诲瀷.
             * @param {Object} value 鍊�.
             */
            'fieldchange',
             ...
             * 鏁版嵁澧炲姞浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 澧炲姞鐨剅ecord.
             * @param {Number} index 鎸囬拡.
             */
            'add',
            /**
             * @event remove
             * 鏁版嵁鍒犻櫎浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 鍒犻櫎鐨剅ecord.
             * @param {Number} index 鎸囬拡.
             */
            'remove',
            /**
             * @event beforeremove
             * 鏁版嵁鍒犻櫎鍓�.濡傛灉涓簍rue鍒欏垹闄や竴鏉¤褰�,false鍒欎笉鍒犻櫎鐩存帴杩斿洖
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Array} records 灏嗚鍒犻櫎鐨勬暟鎹泦鍚�
             */
            'beforeremove',
            /**
             * @event afterremove
             ...
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 鏇存柊鐨剅ecord.
             * @param {String} name 鏇存柊鐨刦ield.
             * @param {Object} value 鏇存柊鐨勫€�.
             * @param {Object} oldvalue 鏇存柊鍓嶇殑鍊�.
             */
            'update',
            /**
             * @event clear
             ...
             * DataSet鏍￠獙浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 鏍￠獙鐨剅ecord.
             * @param {String} name 鏍￠獙鐨刦ield.
             * @param {Boolean} valid 鏍￠獙缁撴灉. true 鏍￠獙鎴愬姛  false 鏍￠獙澶辫触
             */
            'valid',
            /**
             * @event indexchange
             * DataSet褰撳墠鎸囬拡鏀瑰彉浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 褰撳墠record.
             */
            'indexchange',
            /**
             * @event beforeselect
             * 閫夋嫨鏁版嵁鍓嶄簨浠�. 杩斿洖true琛ㄧず鍙互閫変腑,false琛ㄧず涓嶈兘閫変腑
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 閫夋嫨鐨剅ecord.
             */
            'beforeselect',
            /**
             * @event select
             ...
             * 鍙栨秷閫夋嫨鏁版嵁浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 鍙栨秷閫夋嫨鐨剅ecord.
             */
            'unselect',
            /**
             * @event selectall
             ...
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {Aurora.Record} record 鍙栨秷閫夋嫨鐨剅ecord.
             * @param {String} name 閲嶇疆鐨刦ield.
             * @param {Object} value 閲嶇疆鐨勫€�.
             */
            'reject',
            /**
             * @event wait
            ...
            /**
             * @event beforesubmit
             * 鏁版嵁鎻愪氦鍓嶄簨浠�.濡傛灉涓篺alse鍒欎腑鏂彁浜よ姹�
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             */
            'beforesubmit',
            /**
             ...
             * 鏁版嵁鎻愪氦浜嬩欢.
             * @param {Aurora.DataSet} dataSet 褰撳墠DataSet.
             * @param {String} url 鎻愪氦鐨剈rl.
             * @param {Array} datas 鎻愪氦鐨勬暟鎹�.
             */
            'submit',
            /**
             * @event submitsuccess
     ...
     * 鑾峰彇Field閰嶇疆.
     * @param {String} name Field鐨刵ame.
     * @return {Aurora.Record.Field} field閰嶇疆瀵硅薄
     */
...
//        if(this.sortInfo) this.sort();
    ...
    /**
     * 鍒涘缓涓€鏉¤褰�
     * @param {Object} data 鏁版嵁瀵硅薄
     * @param {Number} index 鎸囧畾浣嶇疆.鑻ヤ笉鎸囧畾鍒欐坊鍔犲埌鏈€鍚�.
     * @return {Aurora.Record} record 杩斿洖鍒涘缓鐨剅ecord瀵硅薄
     */
...
//            if(dirty)record.dirty = true;//MAS浜戞柊澧炵壒鎬�
...
//    },
    /**
     * 鏂板鏁版嵁.
     * @param {Aurora.Record} record 闇€瑕佹柊澧炵殑Record瀵硅薄.
     * @param {Number} index 鎸囧畾浣嶇疆.鑻ヤ笉鎸囧畾鍒欐坊鍔犲埌鏈€鍚�.
     */
...
//      }
    ...
    /**
     * 鑾峰彇褰撳墠鎸囬拡鐨凴ecord.
     * @return {Aurora.Record} 褰撳墠鎸囬拡鎵€澶勭殑Record
     */
    ...
    /**
     * 绉婚櫎鏁版嵁.
     * @param {Aurora.Record} record 闇€瑕佺Щ闄ょ殑Record.
     */
...
//        }
    ...
    /**
     * 鑾峰彇褰撳墠鏁版嵁闆嗕笅鐨勬墍鏈夋暟鎹�.
     * @return {Array} records 褰撳墠鏁版嵁闆嗙殑鎵€鏈塕ecord.
     */
    getAll : function(){
        return this.data;
    },
    /**
     * 鏌ユ壘鏁版嵁.
     * @param {String} property 鏌ユ壘鐨勫睘鎬�.
     * @param {Object} value 鏌ユ壘鐨勫睘鎬х殑鍊�.
     * @return {Aurora.Record} 绗﹀悎鏌ユ壘鏉′欢鐨勭涓€涓猺ecord
     */
    find : function(property, value){
        var r = null;
     ...
     * 鏍规嵁id鏌ユ壘鏁版嵁.
     * @param {Number} id id.
     * @return {Aurora.Record} 鏌ユ壘鐨剅ecord
     */
    ...
    /**
     * 閫夋嫨鏌愪釜record.
     * @param {Aurora.Record} record 闇€瑕侀€夋嫨鐨剅ecord.
     */
...
//        if(!this.execSelectFunction(r))return;
    ...
    /**
     * 鍙栨秷閫夋嫨鏌愪釜record.
     * @param {Aurora.Record} record 闇€瑕佸彇娑堥€夋嫨鐨剅ecord.
     */
    ...
    execSelectFunction:function(r){
        if(this.selectfunction){
            ...
            if(selfun == null){
                alert("鏈壘鍒�"+this.selectfunction+"鏂规硶!")
            }else{
...
//          var go = Math.max(0,page-2)*this.pagesize + this.data.length + 1;
    ...
    /**
     * 浠呭dataset鏈韩杩涜鏍￠獙,涓嶆牎楠岀粦瀹氱殑瀛恉ataset.
     * @param {Boolean} selected 鏍￠獙閫変腑鐨勮褰�.
     * @return {Boolean} valid 鏍￠獙缁撴灉.
     */
                ...
                for(var key in dmap){
                    ...
                    if(record.data[key]){
                        ...
                        if(!ds.validate()) {
                            ...
                            ds.reConfig(current.data[key]);//寰幆鏍￠獙瀹屾瘯鍚�,閲嶆柊瀹氫綅鍒板綋鍓嶈
                    }
                }
...
//              }
    ...
    /**
     * 璁剧疆鏌ヨ鐨勫弬鏁�.
     * @param {String} para 鍙傛暟鍚�.
     * @param {Object} value 鍙傛暟鍊�.
     */
    ...
    /**
     * 绛夊緟ds涓殑鎵€鏈塺ecord閮絩eady鍚庢墽琛屽洖璋冨嚱鏁�
     * @param {String} isAll 鍒ゆ柇鎵€鏈夌殑record杩樻槸閫変腑鐨剅ecord
     * @param {Function} callback 鍥炶皟鍑芥暟
     * @param {Object} scope 鍥炶皟鍑芥暟鐨勪綔鐢ㄥ煙
     */
...
//      this.fireBindDataSetEvent("beforeload", this);//涓籨ataset鏃犳暟鎹�,瀛恉ataset涓€鐩磍oading
    ...
    /**
     * 鍒ゆ柇褰撳墠鏁版嵁闆嗘槸鍚﹀彂鐢熸敼鍙�.
     * @return {Boolean} modified 鏄惁鍙戠敓鏀瑰彉.
     */
    isModified : function(){
        var modified = false;
        var records = this.getAll();
...
//    },
    /**
     * 浠son鏍煎紡杩斿洖褰撳墠鏁版嵁闆�.
     * @return {Object} json 杩斿洖鐨刯son瀵硅薄.
     */
...
//            sf.fireEvent("submit", sf);
    ...
    /**
     * 鎻愪氦閫変腑鏁版嵁.
     * @param {String} url(鍙€�) 鎻愪氦鐨剈rl.
     * @param {Array} fields(鍙€�) 鏍规嵁閫夊畾鐨刦ields鎻愪氦.
     */
...
//      this.fireEvent(event,this);
    ...
    commitRecords : function(datas,fire,record){
        ...
        for(var i=0,l=datas.length;i<l;i++){
            ...
            for(var k in data){
                ...
                if(f && f.type == 'dataset'){
                    ...
                    if(ov != nv) {
                        haschange = true;
                        if(fire){
                            //鐢变簬commit鏀惧埌涓婇潰,杩欎釜鏃跺€欎笉鏀瑰彉鐘舵€�,闃叉閲嶅鎻愪氦
                            r.set(field,nv, true);
                        }else{
                            r.data[field] = nv;
            ...
            //鎻愪氦鍚庯紝濡傛灉娌℃湁sequence锛屽氨涓嶄細鏈夊€兼敼鍙橈紝鎵€浠ユ墜鍔ㄨЕ鍙戜竴涓媢pdate銆�
            if(!haschange && fire){
                r.set('__for_update__',true,true);
                delete r.data['__for_update__'];
                r.commit();
            }
//          r.commit();//鎸埌涓婇潰浜�,record.set鐨勬椂鍊欎細瑙﹀彂update浜嬩欢,閲嶆柊娓叉煋.鏈夊彲鑳藉幓鍒ゆ柇isNew鐨勭姸鎬�
        }
    },
...
//      $A.showErrorMessage('閿欒', res.error.message||res.error.stackTrace,null,400,200);
...
//      $A.showWarningMessage('閿欒', res.error.message||res.error.stackTrace,null,350,150);
        this.loading = false;
        $A.SideBar.enable = $A.slideBarEnable;
        this.qtId = null;
    },
...
$A.Record.prototype = {
    ...
    /**
     * 璁剧疆鍊�.
     * @param {String} name 璁惧畾鍊肩殑鍚嶅瓧.
     * @param {Object} value 璁惧畾鐨勫€�.
     * @param {Boolean} notDirty true 涓嶆敼鍙榬ecord鐨刣irty鐘舵€�.
     */
    ...
    /**
     * 璁剧疆鍊�.
     * @param {String} name 鍚嶅瓧.
     * @return {Object} value 鍊�.
     */
    get : function(name){
...
//            this.ds.afterEdit(this);//name,value鎬庝箞澶勭悊?
//        }
//    },
 ...
 * <p>Field鏄竴涓厤缃璞★紝涓昏閰嶇疆鎸囧畾鍒楃殑涓€浜涢檮鍔犲睘鎬э紝渚嬪闈炵┖锛屽彧璇伙紝鍊煎垪琛ㄧ瓑淇℃伅.
 * @constructor
 * @param {Object} data 鏁版嵁瀵硅薄.
 */
...
$A.Record.Field.prototype = {
    ...
    /**
     * 鑾峰彇閰嶇疆淇℃伅
     * @param {String} name 閰嶇疆鍚�
     * @return {Object} value 閰嶇疆鍊�
     */
    get : function(name){
        var v = null;
        if(this.snap){
    ...
    /**
     * 褰撳墠Field鏄惁蹇呰緭.
     * @return {Boolean} required  鏄惁蹇呰緭.
     */
    isRequired : function(){
    ...
    /**
     * 璁剧疆Lov寮瑰嚭绐楀彛鐨凾itle.
     * @param {String} title lov寮瑰嚭绐楀彛鐨凾ile
     */
    setTitle : function(t){
    ...
    /**
     * 璁剧疆Lov寮瑰嚭绐楀彛鐨勫搴�.
     * @param {Number} width lov寮瑰嚭绐楀彛鐨刉idth
     */
    ...
    /**
     * 璁剧疆Lov寮瑰嚭绐楀彛鐨勯珮搴�.
     * @param {Number} height lov寮瑰嚭绐楀彛鐨凥eight
     */
    setLovHeight : function(h){
        this.setPropertity('lovheight',h);
    },
    /**
     * 璁剧疆Lov寮瑰嚭绐楀彛涓璯rid鐨勯珮搴�.
     * 閰嶇疆杩欎釜涓昏鏄敱浜庢煡璇㈡潯浠跺彲鑳藉瓨鍦ㄥ涓紝瀵艰嚧鏌ヨ鐨刦orm杩囬珮.
     * @param {Number} height lov寮瑰嚭绐楀彛鐨刧rid缁勪欢鐨凥eight
     */
    ...
    /**
     * 璁剧疆Lov鐨凪odel瀵硅薄.
     * Lov鐨勯厤缃彲浠ラ€氳繃涓夌鏂瑰紡.(1)model (2)service (3)url.
     * @param {String} model lov閰嶇疆鐨刴odel.
     */
     ...
     * 璁剧疆Lov鐨凷ervice瀵硅薄.
     * Lov鐨勯厤缃彲浠ラ€氳繃涓夌鏂瑰紡.(1)model (2)service (3)url.
     * @param {String} service lov閰嶇疆鐨剆ervice.
     */
     ...
     * 璁剧疆Lov鐨刄rl鍦板潃.
     * Lov鐨勯厤缃彲浠ラ€氳繃涓夌鏂瑰紡.(1)model (2)service (3)url.
     * 閫氳繃url鎵撳紑鐨刲ov锛屽彲浠ヤ笉鐢ㄨ皟鐢╯etLovGridHeight
     * @param {String} url lov鎵撳紑鐨剈rl.
     */
 ...
 * @extends Ext.util.Observable
 * <p>鎵€鏈夌粍浠跺璞＄殑鐖剁被.
 * <p>鎵€鏈夌殑瀛愮被灏嗚嚜鍔ㄧ户鎵緾omponent鐨勬墍鏈夊睘鎬у拰鏂规硶.
 * @author njq.niu@hand-china.com
 * @constructor
 * @param {Object} config 閰嶇疆瀵硅薄.
 */
...
//          this.windowResizeListener();//TODO:浠ュ悗淇敼鏈嶅姟绔痗omponent,鍘绘帀鑷韩灏哄鐨勫垽鏂�
    ...
    initEvents : function(){
        this.addEvents(
             ...
             * 缁勪欢鍊兼敼鍙樹簨浠�.
             * @param {Component} this 褰撳墠缁勪欢.
             * @param {Object} value 鏂扮殑鍊�.
             * @param {Object} oldValue 鏃х殑鍊�.
             */
            'change',
            /**
             * @event valid
             * 缁勪欢楠岃瘉浜嬩欢.
             * @param {Component} this 褰撳墠缁勪欢.
             * @param {Aurora.Record} record record瀵硅薄.
             * @param {String} name 瀵硅薄缁戝畾鐨凬ame.
             * @param {Boolean} isValid 楠岃瘉鏄惁閫氳繃.
             */
            'valid',
            /**
             * @event mouseover
...
//        Ext.getBody().setStyle('overflow','auto');
    ...
    /**
     * 灏嗙粍浠剁粦瀹氬埌鏌愪釜DataSet鐨勬煇涓狥ield涓�.
     * @param {String/Aurora.DataSet} dataSet 缁戝畾鐨凞ataSet. 鍙互鏄叿浣撴煇涓狣ataSet瀵硅薄锛屼篃鍙互鏄煇涓狣ataSet鐨刬d.
     * @param {String} name Field鐨刵ame.
     */
    bind : function(ds, name){
        this.clearBind();
    ...
    /**
     * <p>閿€姣佺粍浠跺璞�.</p>
     * <p>1.鍒犻櫎鎵€鏈夌粦瀹氱殑浜嬩欢.</p>
     * <p>2.浠庡璞＄鐞嗗櫒涓垹闄ゆ敞鍐屼俊鎭�.</p>
     * <p>3.鍒犻櫎dom鑺傜偣.</p>
     */
    destroy : function(){
...
//      if(this.value == value) return;
    ...
    /**
     * 璁剧疆褰撳墠鐨勫€�.
     * @param {Object} value 鍊煎璞�
     * @param {Boolean} silent 鏄惁鏇存柊鍒癲ataSet涓�
     */
...
//          [ou]("mouseout", sf.onMouseOut, sf);
    ...
    initEvents : function(){
        ...
        this.addEvents(
             ...
             * @event keydown
             * 閿洏鎸変笅浜嬩欢.
             * @param {Aurora.Field} field field瀵硅薄.
             * @param {EventObject} e 閿洏浜嬩欢瀵硅薄.
             */
            'keydown',
            /**
             * @event keyup
...
//                return true;
//          }else{
//                this.markInvalid('瀛楁璐规帶');//TODO:娴嬭瘯
//            return false;
//          }
//        }
...
//        this.isSelect = true;
    ...
    dbc2sbc : function(str){
        ...
        for(var i=0;i<str.length;i++){
            ...
            if (code >= 65281 && code <= 65373) {//鍦ㄨ繖涓猽nicode缂栫爜鑼冨洿涓殑鏄墍鏈夌殑鑻辨枃瀛楁瘝宸插強鍚勭瀛楃
                result.push(String.fromCharCode(code - 65248));//鎶婂叏瑙掑瓧绗︾殑unicode缂栫爜杞崲涓哄搴斿崐瑙掑瓧绗︾殑unicode鐮�
...
//              sf.el.blur();
        ...
        createListView : function(datas,binder){
            ...
            for(var i=0,l=datas.length;i<l;i++){
                var d = datas[i];
                sb.push('<tr tabIndex="',i,'"',i%2==1?' class="autocomplete-row-alt"':'','>',this.getRenderText(d,displayFields),'</tr>');  //sf.litp.applyTemplate(d)绛夋暟鎹簮鏄庣‘浠ュ悗鍐嶄慨鏀�
            }
            sb.push('</table>');
            return sb;
        },
...
$A.Button = Ext.extend($A.Component,{
    ...
    initEvents : function(){
        ...
        this.addEvents(
             ...
             * @event click
             * 榧犳爣鐐瑰嚮浜嬩欢.
             * @param {Aurora.Button} button 鎸夐挳瀵硅薄.
             * @param {EventObject} e 閿洏浜嬩欢瀵硅薄.
             */
            'click');
    },
...
$A.CheckBox = Ext.extend($A.Component,{
    ...
    initEvents:function(){
        ...
        this.addEvents(
             ...
             * @event click
             * 榧犳爣鐐瑰嚮浜嬩欢.
             * @param {Aurora.CheckBox} checkBox 鍙€夌粍浠�.
             * @param {Boolean} checked 閫夋嫨鐘舵€�.
             */
            'click');
    },
    destroy : function(){
...
//    this.select(this.selectIndex);
    ...
    initEvents:function(){
        ...
        this.addEvents(
             ...
             * @event click
             * 鐐瑰嚮浜嬩欢.
             * @param {Aurora.Tree} Radio瀵硅薄
             * @param {Object} 褰撳墠閫変腑鐨勫€�
             */
            'click',
            /**
             * @event keydown
...
$A.TriggerField = Ext.extend($A.TextField,{
    ...
    initEvents:function(){
        ...
        this.addEvents(
             ...
             * @event expand
             * 灞曞紑浜嬩欢.
             * @param {Aurora.TriggerField} triggerField 鎵€鏈夊彲灞曞紑鎺т欢瀵硅薄.
             */
            'expand',
            /**
             * @event collapse
    ...
    /**
     * 鍒ゆ柇褰撴椂寮瑰嚭闈㈡澘鏄惁灞曞紑
     * @return {Boolean} isexpanded 鏄惁灞曞紑
     */
    isExpanded : function(){
...
//      Ext.get(document.documentElement).on("mousedown", this.triggerBlur, this, {delay: 10});
        //瀵逛簬鏌愪簺琛屼笂鐨刢b锛屽鏋滄槸浜岀骇鍏宠仈鐨勬儏鍐典笅锛屼細expand澶氭锛屽鑷村娆＄粦瀹氫簨浠�
        Ext.get(document.documentElement).un("mousedown", this.triggerBlur, this);
...
//  },
    ...
    initEvents:function(){
        ...
        this.addEvents(
             ...
             * @event select
             * 閫夋嫨浜嬩欢.
             * @param {Aurora.Combobox} combo combo瀵硅薄.
             * @param {Object} value valueField鐨勫€�.
             * @param {String} display displayField鐨勫€�.
             * @param {Aurora.Record} record 閫変腑鐨凴ecord瀵硅薄
             */
            'select');
    },
//  onTriggerClick : function() {
...
$A.DateField = Ext.extend($A.Component, {
    ...
    initEvents : function(){
        ...
        this.addEvents(
             ...
             * @event select
             * 閫夋嫨浜嬩欢.
             * @param {Aurora.DateField} dateField 鏃ユ湡缁勪欢.
             * @param {Date} date 閫夋嫨鐨勬棩鏈�.
             */
            'select',
            /**
             * @event draw
    ...
    draw: function(date) {
        ...
        //鐢ㄥ綋鏈堢涓€澶╁湪涓€鍛ㄤ腑鐨勬棩鏈熷€间綔涓哄綋鏈堢绗竴澶╃殑澶╂暟,鐢ㄤ笂涓湀鐨勬渶鍚庡ぉ鏁拌ˉ榻�
...
//        window['__host']=null;
    ...
    initEvents : function(){
        ...
        this.addEvents(
             ...
             * @event select
             * 閫夋嫨浜嬩欢.
             * @param {Aurora.DatePicker} datePicker 鏃ユ湡閫夋嫨缁勪欢.
             * @param {Date} date 閫夋嫨鐨勬棩鏈�.
             */
            'select');
    },
...
$A.NavBar = Ext.extend($A.ToolBar,{
    ...
    creatNavInfo : function(){
        ...
        if(sf.type == "simple"){
            var html=[];
            if(totalPage){
                html.push('<span>鍏�'+totalPage+'椤�</span>');
            ...
            html.push('<span>绗�'+currentPage+'椤�</span>');
            return html.join('');
        }else{
...
//        $A.focusWindow = null;
    ...
    initEvents : function(){
        ...
        this.addEvents(
             ...
             * @event beforeclose
             * 绐楀彛鍏抽棴鍓嶇殑浜嬩欢.
             * <p>鐩戝惉鍑芥暟杩斿洖鍊间负false鏃讹紝涓嶆墽琛屽叧闂�</p>
             * @param {Window} this 褰撳墠绐楀彛.         *
             */
            'beforeclose',
            /**
             * @event close
...
//        },10)
     ...
     * 绐楀彛鍔犺浇.
     *
     * @param {String} url  鍔犺浇鐨剈rl
     * @param {Object} params  鍔犺浇鐨勫弬鏁�
     */
 ...
 * @param {Function} callback 鍥炶皟鍑芥暟
 * @param {int} width 瀹藉害
 * @param {int} height 楂樺害
 * @return {Window} 绐楀彛瀵硅薄
 */
 ...
 * @param {String} title 鏍囬
 * @param {String} msg 鍐呭
 * @param {Function} okfun 纭畾鐨刢allback
 * @param {Function} cancelfun 鍙栨秷鐨刢allback
 * @param {int} width 瀹藉害
 * @param {int} height 楂樺害
 * @return {Window} 绐楀彛瀵硅薄
 */
 ...
 * 涓婁紶闄勪欢绐楀彛.
 *
 * @param {String} path  褰撳墠鐨刢ontext璺緞
 * @param {String} title 涓婁紶绐楀彛鏍囬
 * @param {int} pkvalue  pkvalue
 * @param {String} source_type source_type
 * @param {int} max_size 鏈€澶т笂浼犲ぇ灏�(鍗曚綅kb)  0琛ㄧず涓嶉檺鍒�
 * @param {String} file_type 涓婁紶绫诲瀷(*.doc,*.jpg)
 * @param {String} callback 鍥炶皟鍑芥暟鐨勫悕瀛�
 */
...
//        }
        ...
        initEvents : function(){
            ...
            this.addEvents(
                 ...
                 * @event beforecommit
                 * commit涔嬪墠浜嬩欢.
                 * @param {Aurora.Lov} lov 褰撳墠Lov缁勪欢.
                 * @param {Aurora.Record} r1 褰撳墠lov缁戝畾鐨凴ecord
                 * @param {Aurora.Record} r2 閫変腑鐨凴ecord.
                 */
                EVT_BEFORE_COMMIT,
                /**
                 * @event commit
...
//        this.el.setStyle(WIDTH,(w-20)+PX);
        ...
        createListView : function(datas,binder,isRecord){
            ...
            for(var i=0,l=datas.length;i<l;i++){
                var d = datas[i];
                sb.push('<tr tabIndex="',i,'"',i%2==1?' class="autocomplete-row-alt"':_N,'>',this.getRenderText(isRecord?d:new $A.Record(d),displayFields),'</tr>');  //sf.litp.applyTemplate(d)绛夋暟鎹簮鏄庣‘浠ュ悗鍐嶄慨鏀�
            }
            sb.push('</table>');
            return sb;
        },
...
//  },
        onWinClose: function(){
            ...
            sf.isWinOpen = false;
            sf.win = null;
            if(!Ext.isIE6 && !Ext.isIE7){//TODO:涓嶇煡浠€涔堝湴鏂逛細瀵艰嚧鍐茬獊,ie6 ie7 浼氭鎺�
                sf.focus();
            }else{
                    ...
                    if(l>0){
                        if(sf.fetchsingle && l>1){
                            var sb = sf.createListView(datas,binder).join(_N),
                                ...
                                xy = sf.wrap.getXY(),
                                cmp = sf.fetchSingleWindow =  new A.Window({id:sf.id+'_fetchmulti',closeable:true,title:'璇烽€夋嫨', height:Math.min(div.getHeight(),sf.maxHeight),width:Math.max(div.getWidth(),200),x:xy[0],y:xy[1]+sf.wrap.getHeight()});
                            div.remove();
...
$A.MultiLov = Ext.extend($A.Lov,{
    ...
    initEvents : function(){
         ...
         * commit浜嬩欢.
         * @param {Aurora.Lov} lov 褰撳墠Lov缁勪欢.
         * @param {Aurora.Record} r 褰撳墠lov缁戝畾鐨凴ecord
         * @param {Aurora.Record} rs 閫変腑鐨凴ecord闆嗗悎.
         */
    },
    onChange : function(e){},
    onClick : function(e){
...
$A.Customization = Ext.extend(Ext.util.Observable,{
    ...
    mask : function(el){
        var w = el.getWidth();
        var h = el.getHeight();//leftp:0px;top:0px; 鏄惁寮曡捣resize?
        var p = '<div title="鐐瑰嚮璁剧疆涓€у寲" style="border:2px solid #000;cursor:pointer;left:-10000px;top:-10000px;width:'+(w)+'px;height:'+(h)+'px;position: absolute;"><div style="width:100%;height:100%;filter: alpha(opacity=0);opacity: 0;mozopacity: 0;background-color:#ffffff;"> </div></div>';
    ...
    onClick : function(){
        ...
        new Aurora.Window({id:'sys_customization_window', url:context_path + 'modules/sys/sys_customization_window.screen?screen_path='+screen_path + '&id='+ this.cmp.id, title:'涓€у寲璁剧疆',height:170,width:400});
        this.onCmpOut();
    },
    hideMask : function(){
        if(this.masker){
    ...
    A.MultiTextField = Ext.extend(A.TextField,{
        ...
        initEvents : function(){
            ...
            this.addEvents(
                 ...
                 * @event commit
                 * commit浜嬩欢.
                 * @param {Aurora.MultiTextField} multiTextField 褰撳墠MultiTextField缁勪欢.
                 * @param {Aurora.Record} r1 褰撳墠MultiTextField缁戝畾鐨凴ecord
                 * @param {Aurora.Record} r2 閫変腑鐨凴ecord.
                 */
                EVT_COMMIT
            );
                    ...
                    if(res.result.record){
                        ...
                        if(l>0){
                            if(sf.fetchsingle && l>1){
                                var sb = sf.createListView(datas,binder).join(_N),
                                    div = new Ext.Template('<div style="position:absolute;left:0;top:0">{sb}</div>').append(document.body,{'sb':sb},true),
                                    cmp = sf.fetchSingleWindow =  new A.Window({id:sf.id+'_fetchmulti',closeable:true,title:'璇烽€夋嫨', height:Math.min(div.getHeight(),sf.maxHeight),width:Math.max(div.getWidth(),200)});
                                div.remove();
                                cmp.body.update(sb)
                                    .on(EVT_MOUSE_MOVE,sf.onViewMove,sf)
                                    .on('dblclick',function(e,t){
 ...
 * title:娑堟伅鏍囬
 * content锛氭秷鎭唴瀹�
 * confirmReturnFunc锛氱‘瀹氭瘉鎺夊嚱鏁�
 * cancellReturnFunc锛氬彇娑堝洖璋冨嚱鏁�
 */
