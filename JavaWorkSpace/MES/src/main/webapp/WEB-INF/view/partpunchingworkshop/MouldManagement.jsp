    ...
    <form id="fmsave" method="post">
        <div class="fitem" align="center"  >
            <table style="border-collapse:separate; border-spacing:10px 20px; font-size: 15px" id="table">
                ...
                <tr id="number2">
                    <td><span class="mm3">颗/次：</span><input name="Inlocation"  id="number" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                </tr>
                <tr id="temple2">
                    <td><span class="mm3">备 注：</span><input name="Inlocation"  id="temple" class="easyui-textbox" style="width: 150px;height: 25px"   /></td>
                </tr>
            </table>
...
<div id="dlgmmupdate" class="easyui-dialog"  title="模具修改" style="width:330px; height: 500px;  padding: 10px;"
     closed="true" buttons="#dlgmmupdate-buttons">
    <div class="ftitle" align="center" style="font-size: 15px">
        模具修改
    </div>
    <form id="fmupdate" method="post">
        <div class="fitem" align="center"  >
            <table style="border-collapse:separate; border-spacing:10px 20px; font-size: 15px" id="tableupdate">
                <tr id="updateMouldID2">
                    <td>模具编码：<input name="Infname" id="updateMouldID1"  class="easyui-textbox"    style="width: 150px;height: 25px"align="left" /></td>
                </tr>
                <tr id="updateInfname2">
                    <td>模具名称：<input name="Infname" id="updateInfname" class="easyui-textbox" style="width: 150px;height: 25px"  align="left" /></td>
                </tr>
                <tr id="updatepartDrawing2">
                    <td>模具规格：<input name="Infname" id="updatepartDrawing" class="easyui-textbox" style="width: 150px;height: 25px"  align="left" /></td>
                </tr>
                <tr id="updateInlocation2">
                    <td>存放库位：<input name="Inworkcentre2" id="updateInlocation" class="easyui-textbox" style="width: 150px;height: 25px"  align="left" /></td>
                </tr>
                <tr id="updateInTonnage2">
                    <td>冲床吨位：<input name="Inlocation"  id="updateInTonnage" class="easyui-textbox" style="width: 150px;height: 25px"   /></td>
                </tr>
                <tr id="updatenumber2">
                    <td><span class="mm3">颗/次：</span><input name="Inlocation"  id="updatenumber" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                </tr>
                <tr id="updatetemple2">
                    <td><span class="mm3">备 注：</span><input name="Inlocation"  id="updatetemple" class="easyui-textbox" style="width: 150px;height: 25px"   /></td>
                </tr>
            </table>
...
<div id="dlgmmupdate-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="OtherSave1()" iconcls="icon-save">保存</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="Canceldate1()" iconcls="icon-cancel">取消</a>
</div>
...
//                data:$("#login").val(),   //序列化
    ...
    $("#mm").datagrid({
        //单击事件
        onDblClickRow : function (index, row) {  //easyui封装好的事件（被单机行的索引，被单击行的值）

            var department = $('#mmdepartment').combobox('getText');
            if(department!=""){
                ids = row.id;
                $("#updateMouldID1").textbox("setValue",row.mouldID);
                $('#updateInfname').textbox("setValue",row.mouldName);
                $('#updatepartDrawing').textbox("setValue",row.partDrawing);
                $('#updateInlocation').textbox("setValue",row.location);
                $('#updateInTonnage').textbox("setValue",row.tonnage);
                $('#updatenumber').textbox("setValue",row.nunber);
                $('#updatetemple').textbox("setValue",row.remark);
                $("#dlgmmupdate").dialog("open");

            }else {
                $.messager.alert("温馨提示","请选择部门!");
            }
    ...
    function OtherSave1() {
        ...
        var department = $('#mmdepartment').combobox('getText');
        var MouldID = $('#updateMouldID1').val();
        var  fname = $('#updateInfname').val();
        var partDrawing = $('#updatepartDrawing').val();
        var location = $('#updateInlocation').val();
        var Tonnage = $('#updateInTonnage').val();
        var temple = $('#updatetemple').val();
        var number = $('#updatenumber').val();
        var name = localStorage.getItem("User");
        if(MouldID!=""&&fname!=""&&partDrawing!=""&&location!=""&&Tonnage!=""&&number!="") {
            $.ajax({
                    type:"post",
                    url: "${ctxPath}/MouldManage/updateMould.action",//要发送的后台地址
                    data:{
                        year:uyear,
                        month:umonth,
                        name:name,
            ...
            success: function (data) {//ajax请求成功后触发的方法
                ...
                $("#mm").datagrid({
                    ...
                    columns:[[
                        ...
                        {field:'partDrawing',title:'模具规格',width: '100px',align:'left',halign:'center'},
                        ...
                        {field:'remark',title:'备注',width:'60px',align:'center',halign: 'center'},
...
//                    })
    ...
    function mmexport1(){
        ...
        var Location = $('#mm-Location').val();
        var nwx = $('#mm-Nwx').combobox('getText');
        var url = "../${ctxPath}/Export/Export1?plant="+plant+"&department="+department+"&MouldID="+MouldID+"" +
