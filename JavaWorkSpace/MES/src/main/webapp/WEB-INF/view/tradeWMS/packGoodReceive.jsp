        ...
        <div class="easyui-panel" data-options="fit:true">
            <div id="pttoolbar" style="overflow:auto;width:100%;height: 100px">
                ...
                <div class="wu-toolbar-search" style="white-space:nowrap">
                    <table>
                        <tr>
                            <td><label>起始日期：</label><input id="packGoodReceive_start"   class="easyui-datebox"   style="width:100px"></input>&nbsp</td>
                            <td><label>厂编合同：</label><input id="packGoodReceive_horderno" class="easyui-textbox" style="width:100px">&nbsp</td>
                            <td> <label>箱号：</label><input id="packGoodReceive_xh" class="easyui-textbox" style="width:100px">&nbsp</td>
                            <td> <a href="#" class="easyui-linkbutton" style="width: 120px"  onclick="ptquery()" data-options="iconCls:'icon-search'">入库查询</a></td>
                        </tr>
                        <tr>
                            <td><label>结束日期：</label><input id="packGoodReceive_end"   class="easyui-datebox"   style="width:100px"></input&nbsp</td>
                            <td><label>装箱单号：</label><input id="packGoodReceive_zxdh" class="easyui-textbox" style="width:100px">&nbsp</td>
                            <td> <label>库位：</label><input id="packGoodReceive_kw" class="easyui-textbox" style="width:100px">&nbsp</td>
                        </tr>
                    </table>
                </div>
            ...
            <div class="easyui-layout haorooms" style="width:100%;height:80%"   >
                <div class="A"  style="width:100%;height:50%" >
                    <div  class="easyui-layout" data-options="fit:true" >
                        <div style="width: 100%;height:53%" data-options="region:'center'">
                            <table id="packGoodReceive_ptyd" class="easyui-datagrid" style="white-space: nowrap;height: 100%" title="装箱检验" data-options="singleSelect:true"></table>
                        </div>
                    </div>
                </div>
                <div class="C"  style="width:100%;height:50%" >
                    <table class="easyui-datagrid" id="packGoodReceive_pttakegrid" title="成品入库记录" style="white-space: nowrap;height: 100%" data-options="singleSelect:true,onClickRow:onClickRow2,editor:'text'" ></table>
                </div>
            </div>
        </div>
    </div>
...
//                    sortName:'f3',
    ...
    $("#packGoodReceive_pttakegrid").datagrid({
...
//                    sortName:'f3',
    ...
    function ptquery() {
//        $("#packGoodReceive_ptyd").datagrid('loadData', { total: 0, rows: [] });
        $.messager.progress({
            title:'请稍后',
            msg:'页面加载中...'
        })
        jQuery.ajax({
            ...
            url: "/packGoodReceiveTakeController/ptquery.action",//要发送的后台地址
            data: {
                ny:$('#ptdb').val(),
...
//                    sortName:'f3',
    ...
    function ptstoretake() {
        ...
        jQuery.ajax({
            type: "post",//数据发送的方式（post 或者 get）
            url: "/packGoodReceiveTakeController/ptstoretake.action",//要发送的后台地址
            data: {
...
//                alert(editIndex);
                if(editIndex != 'undefined'){
                    $('#packGoodReceive_pttakegrid').datagrid('selectRow',editIndex);
                }
                return true;
            },
            error: function (msg) {//ajax请求失败后触发的方法
...
//        if ($('#packGoodReceive_pttakegrid').datagrid('validateRow', editIndex)){
//            $('#packGoodReceive_pttakegrid').datagrid('endEdit', editIndex);
//            editIndex = undefined;
//            return true;
//        }
        else {
    ...
    function asblycardOnClickRow(index){
        if (ydIndex != index){
            var row = $("#packGoodReceive_ptyd").datagrid("getSelected");
...
//        });
    ...
    function onClickRow2(index){
        if (editIndex !== index){
            ...
            var data = $('#packGoodReceive_pttakegrid').datagrid('getData').rows[editIndex];
...
//        });
    ...
    function Print() {
        var row = $("#packGoodReceive_pttakegrid").datagrid("getSelected");
        if(row!==null){
            row.IsPrint = '1';
            $.ajax({
                type: "post",//数据发送的方式（post 或者 get）
            ...
            $('#packGoodReceive_pttakegrid').datagrid('refreshRow', editIndex);
