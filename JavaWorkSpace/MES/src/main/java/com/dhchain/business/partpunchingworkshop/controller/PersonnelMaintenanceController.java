...
public class PersonnelMaintenanceController {
    ...
    @ResponseBody
    @RequestMapping(value = "/selectDeviceNName",method = {RequestMethod.POST,RequestMethod.GET})
    public JSON selectDeviceNName (HttpServletRequest request) throws UnsupportedEncodingException{
        request.setCharacterEncoding("utf-8");
        String plant = request.getParameter("plant");
        String department = request.getParameter("department");
        BaseEquip baseEquip =  new BaseEquip();
        ...
        List<BaseEquip> list =  personnelMaintenanceService.getEquipD(baseEquip);
        JSONArray jslst = new JSONArray();
    ...
    @ResponseBody
    @RequestMapping(value = "/selectDeviceF",method = {RequestMethod.POST,RequestMethod.GET})//获取设备信息
    public JSON SelectDeviceF (HttpServletRequest request) throws UnsupportedEncodingException{
        request.setCharacterEncoding("utf-8");
        String plant = request.getParameter("plant");
        EQPFault eqpFault =  new EQPFault();
        eqpFault.setPlant(plant);
        List<EQPFault> list =  personnelMaintenanceService.getEquipF(eqpFault);
        JSONArray jslst = new JSONArray();
        ...
        if(list.size()>0){
            for(int i = 0 ; i<list.size();i++){
                ...
                String EquipID = eqpFault1.getEquipID();
                String EquipID1 = EquipID ;
                re.put("EquipID",EquipID1);
                jslst.add(re);
            }
