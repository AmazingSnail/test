package com.dhchain.business.assembleplan.download;
...
//         newFile = new String((dir+"/"+uploadFileFileName).getBytes("UTF-8"),"GBK");
//        logger.error(newFile);
//        newFile = new String((dir+"/"+uploadFileFileName).getBytes("GBK"));
//        logger.error(newFile);
        copy(uploadFile, newFile);
       /* logger.error(new String((dir + "/" + uploadFileFileName).getBytes("ISO-8859-1"), "GBK"));
        logger.error(new String((dir + "/" + uploadFileFileName).getBytes("GBK"), "ISO-8859-1"));
        logger.error(new String((dir + "/" + uploadFileFileName).getBytes("ISO-8859-1"), "UTF-8"));
        logger.error(new String((dir + "/" + uploadFileFileName).getBytes("UTF-8"), "ISO-8859-1"));
        logger.error(new String((dir + "/" + uploadFileFileName).getBytes(), "ISO-8859-1"));
        logger.error(new String((dir+"/"+uploadFileFileName).getBytes("ISO-8859-1")));*/
    }
    ...
    public static void copy(File from,File dir) throws IOException {
        BufferedInputStream bis = null;
        BufferedOutputStream bos = null;
        ...
        bos = new BufferedOutputStream(
                new FileOutputStream(
                        new File(dir.getPath() + "/" + from.getName())));
        byte[] buf = new byte[1024*1024];
        int len = -1;
        while ((len = bis.read(buf))!=-1){
            bos.write(buf,0,len);
    ...
    public static HttpServletResponse copy(InputStream from,HttpServletResponse response) throws Exception{ //下载
...
//	}
     ...
     * 计算long成 MB KB B
     * @param l 长度
     * @return  计算long成 MB KB B并转换成String类型
     */
...
//    	try {
//            File dir = new File(StringContext.INTERNALVERSIONPATH);
//            if(!dir.exists()){
//                dir.mkdirs();
//            }
...
//    	try {
//            File dir = new File(StringContext.EXTERNALVERSIONPATH);
//            if(!dir.exists()){
//                dir.mkdirs();
//            }
