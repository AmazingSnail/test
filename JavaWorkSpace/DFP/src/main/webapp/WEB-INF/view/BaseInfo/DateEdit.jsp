...
<div class="easyui-layout" data-options="fit:true">
    <div class="easyui-panel"title="添加/修改主数据" data-options="fit:true">
        <div id="pttoolbar" style="overflow:auto;width:100%;height:100%">
            ...
            <div class="wu-toolbar-search" style="white-space:nowrap;margin-top: 1%;text-align: left">
                ...
                <table style="border-collapse:separate; border-spacing:10px 20px; font-size: 15px;text-align: center" >
                    <tr>
                        <td><span class="bi1">厂内机型</span>：<input name="Infname" id="ZMACH" class="easyui-textbox" style="width: 150px;height: 25px"  align="left"  /></td>
                    </tr>
                    <tr>
                        <td><span class="bi2">类型</span>：<input name="Inlocation"  id="Type" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td><span class="bi2">机型</span>：<input name="Inlocation"  id="Model" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td><span class="bi2">轴距</span>：<input name="Inlocation"  id="Wheelbase" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td><span class="bi1">外廓尺寸</span>：<input name="Inlocation"  id="Outsize" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td><span class="bi3">标定牵引力</span>：<input name="Inlocation"  id="DemarcatedForce" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td>最小使用质量：<input name="Inlocation"  id="MinUse" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td><span class="bi1">执行标准</span>：<input name="Inlocation"  id="ExecStd" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                </table>
                </form>
            </div>
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                ...
                if(window.parent.idm!=''){
                    ...
                    $('#ZMACH').textbox("setValue",window.parent.ZMACH);
                    $('#Type').textbox("setValue",window.parent.Type);
                    $('#Model').textbox("setValue",window.parent.Model);
                    $('#Wheelbase').textbox("setValue",window.parent.Wheelbase);
                    $('#Outsize').textbox("setValue",window.parent.Outsize);
                    $('#DemarcatedForce').textbox("setValue",window.parent.DemarcatedForce);
                    $('#MinUse').textbox("setValue",window.parent.MinUse);
                    $('#ExecStd').textbox("setValue",window.parent.ExecStd);
                }else {
                    $('#ZMACH').textbox("setValue",'');
                    $('#Type').textbox("setValue",'');
                    $('#Model').textbox("setValue",'');
                    $('#Wheelbase').textbox("setValue",'');
                    $('#Outsize').textbox("setValue",'');
                    $('#DemarcatedForce').textbox("setValue",'');
                    $('#MinUse').textbox("setValue",'');
                    $('#ExecStd').textbox("setValue",'');
                }
            }
        });
    }

    function DateEditCancel() {
        j=1;
        idm = '';
    ...
    function Savedate() {
        ...
        var  DemarcatedForce = $('#DemarcatedForce').val();
        var  MinUse = $('#MinUse').val();
        var  ExecStd = $('#ExecStd').val();
        if(ZMACH!=""||Type!=""||Model!=""||Wheelbase!=""||
            Outsize!=""||DemarcatedForce!=""||MinUse!=""||ExecStd!=""){
            jQuery.ajax({
                type: "POST",   //post提交方式默认是get
                url:'${pageContext.request.contextPath}/Tractor/Insert1.action',
                dataType: "json",
                data: {
                    time:time,
                    name:name,
