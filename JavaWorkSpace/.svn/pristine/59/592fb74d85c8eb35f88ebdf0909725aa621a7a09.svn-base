        ...
        <div id="pttoolbar" style="overflow:auto;width:100%;height:100%">
            ...
            <div class="wu-toolbar-search" style="white-space:nowrap;margin-top: 1%;text-align: left">
                <table style="border-collapse:separate; border-spacing:10px 20px; font-size: 15px;text-align: center" >
                    <tr>
                        <td><span class="bi1">厂内机型</span>：<input name="Infname" id="ZMACH3" class="easyui-textbox" style="width: 150px;height: 25px"  align="left"  /></td>
                    </tr>
                    <tr>
                        <td><span class="bi3">发动机型号</span>：<input name="Inlocation"  id="ZCHYJ" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                    <tr>
                        <td><span class="bi2">功率</span>：<input name="Inlocation"  id="Power" class="easyui-textbox" style="width: 150px;height: 25px"    /></td>
                    </tr>
                </table>
            </div>
        </div>
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                ...
                if(window.parent.idm!=''){
                    ...
                    $('#ZMACH3').textbox("setValue",window.parent.ZMACH);
                    $('#ZCHYJ').textbox("setValue",window.parent.ZCHYJ);
                    $('#Power').textbox("setValue",window.parent.Power);
                }else {
                    $('#ZMACH3').textbox("setValue",'');
    ...
    function Save1() {
        var  ZMACH = $('#ZMACH3').val();
        var  ZCHYJ = $('#ZCHYJ').val();
        var  Power = $('#Power').val();
        if(ZMACH!=''||ZCHYJ!=''||Power!=''){
            jQuery.ajax({
                type: "POST",   //post提交方式默认是get
                url:'${pageContext.request.contextPath}/Tractor/Insert2.action',
                dataType: "json",
                data: {
                    time:time,
                    name:name,
