...
<table id="dg" title="部署管理" class="easyui-datagrid"
  ...
  url="${pageContext.request.contextPath}/deploy/deployPage.action" fit="true" toolbar="#depM_tb">
 <thead>
 	<tr>
...
<div id="depM_dlg" class="easyui-dialog" style="width: 320px;height: 150px;padding: 10px 20px" closed="true" buttons="#dlg-buttons">
 
 	<form id="depM_fm" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
    ...
    function searchDeploy(){
        $("#depM_dg").datagrid('load',{
            "s_name":$("#s_name").val()
        });
    }
    function openDeployAddDiglog(){
        $("#depM_dlg").dialog("open").dialog("setTitle","添加流程部署");
        url="${pageContext.request.contextPath}/deploy/bpmnDeploy.action"
    }
    function saveDeploy(){
//        $("#depM_fm").form("submit",{
//            url:url,
...
//                    $("#depM_dlg").dialog("close");
//                    $("#depM_dg").datagrid("reload");
//                }else{
...
//            }
//        });
        $.messager.alert("系统系统","调整为后台部署");
    }
    function closeDeployDialog(){
        $("#depM_dlg").dialog("close");
    }
    function deleteDeploy(){
        var selectRows=$("#depM_dg").datagrid("getSelections");
