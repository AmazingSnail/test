    ...
    <div class="easyui-panel" title="设备维修管理" data-options="fit:true">
        <div id="pttoolbar" style="overflow:auto;width:100%;height: 80px">
            ...
            <div class="wu-toolbar-search" style="white-space:nowrap">
                <table>
                    <tr>
                        <div id="test2"><input  id ="he2" style="width:110px" class="easyui-text" hidden="hidden"></div>
                        <td><label><span class="w3">事业部</span>：</label>
                            <select class="easyui-combobox" name="mmPlant" style="width:100px" datatype="text" id="hePlant" panelHeight="auto" data-options="
                                valueField:'Fremark',
                                textField:'Fname',
                                panelHeight:'250'"
                                    style="width:100px">
                            </select></td>
                        <td><label>起始时间：</label><input  id ="he1" style="width:110px" class="easyui-datebox"></td>
                        <td><label>维修人员：</label>
                            <select class="easyui-combobox" name="mmPlant" style="width:100px" datatype="text" id="Username1" panelHeight="auto" data-options="
                                valueField:'Username',
                                textField:'Username',
                                panelHeight:'250'"
                                    style="width:100px">
                            </select></td>
                        <td><label>检修状态：</label> <select class="easyui-combobox" name="Pkgin-FnameStr" datatype="text" id="emSerStatus1" panelHeight="auto" style="width:100px">
                            <option value="">   </option>
                        ...
                        <td><a href="#" class="easyui-linkbutton"  onclick="hequery()" data-options="iconCls:'icon-search'">查询</a></td>
                    </tr>
                    <tr>
                        <td><label><span class="w2">部门</span>：</label>
                            <select class="easyui-combobox" name="mmPlant" style="width:100px" datatype="text" id="emdepartment1" panelHeight="auto" data-options="
                                valueField:'DepartMent',
                                textField:'DepartMent',
                                panelHeight:'250'"
                                    style="width:100px">
                            </select></td>
                        <td><label>结束时间：</label><input  id ="hee" style="width:110px" class="easyui-datebox"></td>
                        <td> <label>故障类型：</label> <select class="easyui-combobox" name="Pkgin-FnameStr" datatype="text" id="emFaultTypes1" panelHeight="auto" style="width:100px">
                            <option value="">   </option>
                            <option value="GZ01">电气故障</option>
                            <option value="GZ02">机械故障</option>
                            <option value="GZ03">模具故障</option>
                        </select></td>
                        <td> <label>设备编号： </label><input id="emEquipID1" class="easyui-textbox" style="width:100px"></td>
                    </tr>
                </table>
            </div>
        </div>
            ...
            <div class="A" data-options="region:'west',split:true" >
                <div  class="easyui-layout" data-options="fit:true" style="width:100%;height:65%">
                    <div style="width: 100%;height:53%" data-options="region:'center'">
                        <table id="he" class="easyui-datagrid" style="white-space: nowrap;height: 100%"  title="故障明细（检修状态：绿色:检修完成，浅绿色：已完成,
                           灰色：检修中，红色：应答中)" data-options="singleSelect:true"></table>
                    </div>
                </div>
            </div>
            <div class="C" title="故障分析报告" id="report1" data-options="region:'east',split:true" hidden="hidden" >
                <div style="height: 20px"></div>
                <label style="font-size: 15px;margin-left: 10px">故障描述及原因分析：</label>
                <div style="height: 20px"></div>
                <textarea style="border:solid 10px #f4f5f7;width:80%;height:20%;resize:none ;margin-left: 20px" id="Fault1"></textarea>
                <div style="height: 20px"></div>
                <label style="font-size: 15px;margin-left: 10px">处理结果：</label>
                <div style="height: 20px"></div>
                <textarea style="border:solid 10px #f4f5f7;width:80%;height:20%;resize:none;margin-left: 20px" id="Results1"></textarea>
                <div style="height: 20px"></div>
                <div style="width:80%">
...
<script type="text/javascript">
    var redepartment ='';
    function SaveRe () {
        var row = $('#he').datagrid('getSelected');
        var s = row.statusName1;
        var faultid = row.faultID;
        var EquipID = row.equipID;
        var plant = $('#hePlant').combobox('getText');
        var department = $('#emdepartment1').combobox('getText');
        var username = localStorage.getItem("User");
        var Fault=$("#Fault1").val();
...
//                $("#Results").val(Results);
    ...
    $(function () {
        ...
        $('#he1').val(year+"-"+month+"-"+day);
        $('#hee').val(year+"-"+month+"-"+day);
        $('#he2').val(year+"-"+month+"-"+day);
        var db = $('#he2');
        db.datebox({
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                $('#hePlant').combobox('loadData', data);
                $("#report1" ).hide();
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                $('#emdepartment1').combobox('loadData', data);
                redepartment =  $('#emdepartment1').combobox('getValue');
            }
        });
        jQuery.ajax({
            type: "POST",   //post提交方式默认是get
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                $('#Username1').combobox('loadData', data);
            }
        });
//
    ...
    function hequery() {
        ...
        var plant = $('#hePlant').combobox('getText');
        var department = $('#emdepartment1').combobox('getText');
        var Username = $('#Username1').combobox('getText');
        var emSerStatus = $('#emSerStatus1').combobox('getValue');
        var emFaultTypes = $('#emFaultTypes1').combobox('getValue');
        var EquipID = $('#emEquipID1').val();
        var starttime = $('#he1').datebox('getValue');
        var endtime = $('#hee').datebox('getValue');
        var fdata;
        if(redepartment==''||redepartment == department){
            $.ajax({
            ...
            $('#he').datagrid('loadData',{total:0,rows:[]});
        }else {
            $.messager.alert('温馨提示', '不允许跨部门操作!!!');
        }
