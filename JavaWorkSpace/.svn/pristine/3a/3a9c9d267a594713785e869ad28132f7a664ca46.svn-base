    ...
    <div class="easyui-panel" title="质量管理" data-options="fit:true">
        <div id="pttoolbar" style="overflow:auto;width:100%;height: 80px">
            <div class="wu-toolbar-button">
                <div id="testhh"><input  id ="hhqadb2" style="width:110px" class="easyui-text" hidden="hidden"></div>
                ...
                <table>
                    <tr>
                        <td><label><span class="w3">事业部</span>：</label>
                            <select class="easyui-combobox" style="width:100px" name="MonthPlan-Plant" datatype="text" id="hhqaPlant" panelHeight="auto" data-options="
                                valueField:'Fremark',
                                textField:'Fname',
                                panelHeight:'250'"
                                    style="width:100px">
                            </select></td>
                        <td><label>起始时间：</label><input  id ="hhqadb1" style="width:110px" class="easyui-datebox"></td>
                        <td><label>设备编号：</label><select class="easyui-combobox" style="width:100px" name="MonthPlan-Plant" datatype="text" id="hhqa-EquipID" panelHeight="auto" data-options="
                                valueField:'EquipID',
                                textField:'EquipID',
                                panelHeight:'250'"
                                                        style="width:100px">
                        </select></td>
                        <td><label>送检类型：</label><select class="easyui-combobox" name="Pkgin-FnameStr" datatype="text" id="hhqa-CheakTypes" panelHeight="auto" style="width:100px">
                            <option value=""> </option>
                            <option value="自检">自检</option>
                            <option value="送检">送检</option>
                        </select> </td>
                        <td><a href="#" class="easyui-linkbutton"  onclick="hhqaquery()" style="width: 100px" data-options="iconCls:'icon-search'">查询</a></td>
                    </tr>
                    <tr>
                        <td><label><span class="w2">部门</span>：</label>
                            <select class="easyui-combobox" name="mmPlant" style="width:100px" datatype="text" id="hhqadepartment" panelHeight="auto" data-options="
                                valueField:'DepartMent',
                                textField:'DepartMent',
                                panelHeight:'250'"
                                    style="width:100px">
                            </select></td>
                        <td><label>结束时间：</label><input  id ="hhqadb" style="width:110px" class="easyui-datebox"></td>
                        <td> <label>送检单号：</label><input id="hhqa-CheakID" class="easyui-textbox" style="width:100px"></td>
                        <td> <label>任务编号：</label><input id="hhqa-PlanID" class="easyui-textbox" style="width:100px"></td>
                    </tr>
                </table>
            </div>
        </div>
        <div class="easyui-layout haorooms"  >
            <div class="hhqa" data-options="region:'west',split:true" >
                <div  class="easyui-layout" data-options="fit:true" style="width:100%;height:65%">
                    <div style="width: 100%;height:53%" data-options="region:'center'">
                        <table id="hhqa" class="easyui-datagrid" style="white-space: nowrap;height: 100%"  data-options="singleSelect:true"></table>
                    </div>
                </div>
            </div>
            <div class="hhqc"  data-options="region:'east',split:true">
                <table class="easyui-datagrid" id="hhqaid"   style="white-space: nowrap;height: 100%" data-options="singleSelect:true,editor:'text'" ></table>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $("#testhh" ).css("display", "none");
    $(function () {
        var date=new Date();
        var year = date.getFullYear();
        var month = date.getMonth()+1;
        var day = date.getDate();
        $('#hhqadb1').val(year+"-"+month+"-"+day);
        $('#hhqadb').val(year+"-"+month+"-"+day);
        $('#hhqadb2').val(year+"-"+month+"-"+day);
        var db = $('#hhqadb2');
        db.datebox({
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                $('#hhqaPlant').combobox('loadData', data);
                //alert(11111);
...
//                data:$("#login").val(),   //序列化
            ...
            success: function (data) {
                $('#hhqadepartment').combobox('loadData', data);
            }
        });


    });
    $('#hhqadepartment').combobox({
        onChange: function (newValue, oldValue) {
            $('#hhqa-EquipID').combobox('clear');
...
//                data:$("#login").val(),   //序列化
                ...
                success: function (data) {
                    $('#hhqa-EquipID').combobox('loadData', data);
                }
            });
        }
    });
    ...
    function hhqaquery() {
        var plant = $('#hhqaPlant').combobox('getText');
        var deparment = $('#hhqadepartment').combobox('getText');
        var CheakTypes = $('#hhqa-CheakTypes').combobox('getText');
        var starttime = $('#hhqadb1').datebox('getValue');
        var endtime = $('#hhqadb').datebox('getValue');
        var EquipID = $('#hhqa-EquipID').combobox('getText');
        var CheakID = $('#hhqa-CheakID').val();
        var PlanID = $('#hhqa-PlanID').val();
        var fdata;
            ...
            success: function (data) {//ajax请求成功后触发的方法
                ...
                $("#hhqa").datagrid({
                    ...
                    columns:[[
                        ...
                        {field:'provingTime',title:'检验时间段',width: '160px',align:'center',halign: 'center'},
...
//                });
        ...
        $('#hhqa').datagrid('loadData',{total:0,rows:[]});
